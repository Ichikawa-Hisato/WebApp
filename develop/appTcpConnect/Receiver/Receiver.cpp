...
int main(int argc, char *argv[])
{
    ...
    //ソケットの生成
    int sockfd = socket(addr.sin_family, SOCK_STREAM, 0); //アドレスドメイン, ソケットタイプ, プロトコル
    if(sockfd < 0){ //エラー処理
    ...
    if(bind(sockfd, (struct sockaddr *)&addr, sizeof(addr)) < 0){ //ソケット, アドレスポインタ, アドレスサイズ //エラー処理
        std::cout << "Error bind:" << std::endl; //標準出力
        exit(1); //異常終了
    }   

    //受信待ち
    if(listen(sockfd,SOMAXCONN) < 0){ //ソケット, キューの最大長 //エラー処理
        std::cout << "Error listen:" << std::endl; //標準出力
    ...
    if(connect < 0){ //エラー処理
        std::cout << "Error accept:" << std::endl; //標準出力
        exit(1); //異常終了
    }
